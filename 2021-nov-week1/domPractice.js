window.onload = function () {
  ...
  console.log(
    "h1의 ownerDocument는 document인가요? : " +
      (h1.ownerDocument === window.document)
  );
  console.log(
    "h1과 p의 ownerDocument는 같다? : " +
      (h1.ownerDocument.ownerDocument === p1.ownerDocument.ownerDocument)
  );

  // NodeList
  const nodeList = document.querySelectorAll("p");
  console.log(
    "querySelectorAll은 NodeList를 리턴합니다: ",
  ...
  console.log(table);
  const tableAttrs = table.attributes; // Element interface
  for (let i = 0; i < tableAttrs.length; i++) {
    if (tableAttrs[i].nodeName.toLowerCase() === "border") {
      table.border = "1"; // HTMLTableElement interface
    }
  }
};
